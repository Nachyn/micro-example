    ...
    public class Startup
    {
        ...
        private static void AddCatalogClient(IServiceCollection services)
        {
            ...
            services.AddHttpClient<CatalogClient>(client => { client.BaseAddress = new Uri("https://localhost:5001"); })
                ...
                {
                    return builder.Or<TimeoutRejectedException>(
                    ).WaitAndRetryAsync(3,
                        ...
                        (outcome, timespan, retryAttempt) =>
                        {
                            ...
                            serviceProvider.GetService<ILogger<CatalogClient>>()
                                ?.LogWarning($"Delaying for {timespan.Seconds} seconds, then making retry {retryAttempt}");
                        });
                    ...
                    .CircuitBreakerAsync(
                        ...
                        (outcome, timeSpan) =>
                        {
                            var serviceProvider = services.BuildServiceProvider();
                            serviceProvider.GetService<ILogger<CatalogClient>>()
                                ?.LogWarning($"Opening the curcuit for {timeSpan.TotalSeconds}...");
                        },
                        () =>
                        {
                            var serviceProvider = services.BuildServiceProvider();
                            serviceProvider.GetService<ILogger<CatalogClient>>()
                                ?.LogWarning("Closing the curcuit...");
                        }
                    ))
